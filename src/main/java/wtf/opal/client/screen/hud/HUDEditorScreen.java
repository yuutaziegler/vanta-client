...
extends class_437 { 
    ...
    public void method_25394(class_332 context, int mouseX, int mouseY, float delta) { 
        ...
        if (hudSettings != null && hudSettings.isEnabled()) { 
            LiquidGlassRenderer.drawGlassPanel(bannerX, bannerY, bannerW, bannerH, 6.0f); 
        } else { 
        ...
        // Apply liquid glass to reset button 
        if (hudSettings != null && hudSettings.isEnabled()) { 
            LiquidGlassRenderer.drawGlassPanel(rBtnX, rBtnY, rBtnW, rBtnH, 4.0f); 
        } else { 
        ...
        if (overlayModule != null) { 
            ...
            for (IOverlayElement element : elements) { 
                ...
                // Apply liquid glass to element background 
                if (hudSettings != null && hudSettings.isEnabled()) { 
                ...
                if ((isHover = HoverUtility.isHovering(x, y, width, height, mouseX, mouseY)) || prop.isDragging()) { 
                    // Highlight selected/dragging element 
                    ...
                    // Show resize handles on corners when not dragging 
                    if (!prop.isDragging()) { 
                        float gripSize = 6.0f; 
                        int gripColor = -16718337; 
                        // Top-left grip 
                        NVGRenderer.roundedRect(x - gripSize, y - gripSize, gripSize, gripSize, 3.0f, gripColor); 
                        // Top-right grip 
                        NVGRenderer.roundedRect(x + width - gripSize, y - gripSize, gripSize, gripSize, 3.0f, gripColor); 
                        // Bottom-left grip 
                        NVGRenderer.roundedRect(x - gripSize, y + height - gripSize, gripSize, gripSize, 3.0f, gripColor); 
                        // Bottom-right grip 
                        NVGRenderer.roundedRect(x + width - gripSize, y + height - gripSize, gripSize, gripSize, 3.0f, gripColor); 
                    } 
                    continue; 
                } 
                // Default element outline with glass style 
